...
4. After the installation is finished you can change some configuration settings if you want, but for starting it is not really necessary.
...
Be aware of namespaces!! If you are using a namespace on the top of your template file, you have to adapt the instantiation of the class by using the FrontendForms namespace. 
This module runs in its own namespace called FrontendForms. Take a look at the following example:

```php

// if you are not using a namespace on your template file, instantiating a class without using the FrontendForms namespace will be fine
...
// you have to do it on every class instantiation (fe input field, select field,..) not only on the Form class as in the example above
```
...
## IP-banning
Add IP-addresses to a black list to prevent them accessing your forms. If the visitor's IP is on this list, an alert box will be displayed,
which informs the visitor, that his IP is on the black list. The form itself will not be displayed.
...
This means that visitors with an IP listed in the module configuration are not able to view a form
...
```
By visiting the page, an alertbox will be displayed on the frontend instead of the form. Try it out to see how it works.
...
## Default fields
To make life a little easier I have created the most used fields in forms as pre-defined default fields with its own class.
These are: 
...
This is the same as the example above but shorter and nicer.
...
First you need to know, that inside the email_templates folder you will PHP files with various names (fe template_1.php, template_2.php).
